...
queue_t* create_queue(unsigned int _capacity){
	queue_t* myQueue = malloc(sizeof(queue_t)); //Allocates memory for instance of queue
	myQueue->size = 0; //Number of elements within the queue
	myQueue->capacity = _capacity; //Size determined based of desired amount
	myQueue->data = malloc(sizeof(int)* _capacity); //Allocates memory for values stored within each position of the queue
	myQueue->back = 0; //Starting position of back of queue
	myQueue->front =0; //Starting position of front of queue

	return myQueue; //Returns constructed instance of a queue
}
...
int queue_enqueue(queue_t* q, int item){
	if (queue_full(q))
		{return -1;}
	q->data[q->back] = item; //Updates que to the value of passed in int item value
	q->size ++; //Increases size by 1
	q->back = (q->back+1)% (q->capacity); //Repositions pointer of que
...
int queue_dequeue(queue_t *q){
	...
	if (queue_empty(q)){exit(1);} //Checks to make sure queue is not empty

	item = q->data[q->front]; //Assigns item to value at head of queue

	q->front = (q->front +1) % (q->capacity); //Updates position of head by one 
											 //Uses mod function to for circular queue functioning

	q->size --; //Decreases size of queue by one 
...
void free_queue(queue_t* q){
	...
	if (q->data !=NULL){
		free(q->data); //Frees the data array from queue
	}
