...
//!!!!!!!!!!!!!!!!NOTE TO SELF!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//index = i * COLUMNS + j This general formula is how we traverse the 1D representation of our array
    //i ==Row 
    //i * columns == Creates the matrix
    // +j == How many columns to traverse within the row
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
/*  ============= Tutorial on Graph format ============
    ...
    The number of connections a node has out is its 'out-degree' (Rows = out degrees)
    The number of connections a node has in are it's 'in-degree' (Columns = in degrees)
*/
...
int nodeInDegree(int* g, int node){
    ...
    for (int i = 0; i < ROWS; i ++){
        // i * colums == isolates the row (Skip these number of elements within the one d array)
        // node == which column (Skip these number of elements within the 1d array)
...
int nodeOutDegree(int* g, int node){
    ...
    //I traverses rows until we reach the node we want to index
    for (int i = 0; i < ROWS; i ++){
        //If we reach that node, we traverse across the row evaluating all columns for number one
        //A one within the row represents an out degree
        if (i ==node){
