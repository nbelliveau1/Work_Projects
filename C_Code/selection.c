...
void swap(int* a, int* b){
  ...
  *a = *b; //Copy the value pointed to b into the memory location pointed to by a
  *b = temp; //Cpy the orginal "a" value into the memory location pointed by b
}
...
int findMinimum(int* array, int start, int stop){
  ...
  int min_Value = start; //Minimal value to start will
    for (int j = start +1; j < stop; j ++ ){
      if (array[j] < array[min_Value]){ //This works because we can assume that the starting point is the smallest value then compare to all others
        min_Value = j; //Updates min_Value to j position if another element within array is smaller than starting point
      }
    }
  return min_Value; 
  }
...
void sortIntegers(int* array, unsigned int size){
  ...
  //Tracks the current position where the next smallest element should go
  for (int i = 0; i < size-1; i ++){
    int min_val = i; //Current lowest value before iteration

    //Searches the rest of the array to find the smallest element
    for (int j = i+1; j<size; j++){
      if (array[j]<array[min_val])
      {
        min_val = j; //Identifies lowest value within iteraltion
      }
    }
  { 
  
  //Happens once per outer iteration, only after identifying the smallest element’s index
  if (min_val!=i)
...
int main(int argc, char* argv[]){
  ...
  // mininul value function
  int dataset7[] = {12,20,30,40,50, 2};
  printf("The smallest value within your dataset is at position %d \n", findMinimum(dataset7, 0, 6));
